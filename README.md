...
- [DFlash: Block Diffusion for Flash Speculative Decoding](<diffusion/6. DFlash Block Diffusion for Flash Speculative Decoding.md>)：DFlash 把 diffusion model 限定为 speculative decoding 的并行 drafter，并用目标自回归模型的 hidden features 与验证步骤同时提高接受长度和保证 lossless 输出。
- [SwiftFusion: Scalable Sequence Parallelism for Distributed Inference of Diffusion Transformers on GPUs](<diffusion/7. SwiftFusion Scalable Sequence Parallelism for Distributed Inference of Diffusion Transformers on GPUs.md>)：SwiftFusion 针对高分辨率图像和长视频 DiT 推理，把低通信量的 Ulysses 用在跨机器路径、把 Ring 留给机器内互联，并用 Torus Attention 与 one-sided communication 隐藏通信和同步开销。
- [Orca: The World is in Your Mind](<diffusion/8. Orca The World is in Your Mind.md>)：Orca 把 world model 的中心目标从 next-token / next-frame / next-action 改写为 next-state prediction，通过无意识学习和有意识学习预训练统一 world latent，再用冻结 backbone 的语言、图像和动作 readout 验证状态转移表征是否可迁移。
